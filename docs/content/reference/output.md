...
describes what users get. Pick a format with `-o`, or let tapas choose:
a table when writing to a terminal, JSONL when piped.

## Formats

```bash
tapas <command> -o table   # aligned columns for reading
tapas <command> -o jsonl   # one JSON object per line, for piping
tapas <command> -o json    # a single JSON array
tapas <command> -o csv     # spreadsheet friendly
tapas <command> -o tsv     # tab-separated
tapas <command> -o url     # just the URL column
tapas <command> -o raw     # the underlying bytes, unformatted
```
...
```bash
tapas <command> --fields id,title,url
```
...
```bash
tapas <command> --template '{{.URL}} {{.Title}}'
```
...
```bash
tapas <command>            # a table, because this is a terminal
tapas <command> | wc -l    # JSONL, because this is a pipe
```
